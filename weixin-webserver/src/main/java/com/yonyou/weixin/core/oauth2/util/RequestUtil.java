	...
	public static JSONObject HttpRequest(String request , String RequestMethod , String output ){
		...
		try {
			...
			if(output!=null){
				...
				out.write(output.getBytes(APPConstants.APP_ENCODING));
				out.close();
			}
			//流处理
			InputStream input = connection.getInputStream();
			InputStreamReader inputReader = new InputStreamReader(input,APPConstants.APP_ENCODING);
...
//	 获取access_token的接口地址（GET）   
	public final static String access_token_url = "https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid=CorpID&corpsecret=SECRET&debug=1";  
	...
	public static String URLEncoder(String str){
		String result = str ;
		try {
		result = java.net.URLEncoder.encode(result,APPConstants.APP_ENCODING);	
		} catch (Exception e) {
        e.printStackTrace();
		}
		return result;
    ...
    public static int PostMessage(String access_token ,String RequestMt , String RequestURL , String outstr){
    	...
    	JSONObject jsonobject = HttpRequest(RequestURL+"&debug=1", RequestMt, outstr);
    	 if (null != jsonobject) {  
