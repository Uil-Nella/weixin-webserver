...
public class MPLoginController {
	...
	private static final Logger logger = Logger
			.getLogger(MPLoginController.class);
	...
	public String login(HttpServletRequest request, Model model)
		...
		if (logger.isInfoEnabled()) {
			logger.info("login(HttpServletRequest, Model) - 自动登录MP平台");
		}
		...
		if (StringUtils.isNotEmpty(staff.getPdomaincode())) {
			String url = MP_SERVER_IP
					+ "?"
					+ MP_WX_SING
					+ "="
					+ HexUtil.bytesToHex(MPUtils.encryptDES((staff
							.getPdomaincode() + "|" + String.valueOf(System
							.currentTimeMillis()))));
